...
function ScoreRing({ score }: { score: number }) {
  const colour =
    score >= 1400 ? "text-emerald-600" : score >= 1100 ? "text-navy-light" : "text-orange-500";
  return (
    <div className="flex flex-col items-center justify-center">
      <span className={`text-3xl font-bold tabular-nums ${colour}`}>{score}</span>
      <span className="text-xs text-text-muted mt-0.5">/ 1600</span>
    </div>
  );
}
...
function SubScore({ label, value }: { label: string; value?: number }) {
  return (
    <div className="flex flex-col items-center bg-surface rounded-lg px-3 py-1.5">
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-white">
        {paymentSuccess && (
          <div className="bg-green-50 border-b border-green-200 py-3 px-6 text-center text-sm text-green-600 font-medium">
            {t("paymentSuccess")}
          </div>
        )}
          ...
          <div className="max-w-4xl mx-auto px-6 pt-10 pb-8">
            ...
            <div className="flex items-center gap-5 mb-8">
              {user?.imageUrl ? (
                ...
                <img
                  src={user.imageUrl}
                  alt="Profile"
                  className="w-20 h-20 rounded-full border-2 border-navy/20 shadow-lg shadow-navy/5"
                />
              ) : (
                <div className="w-20 h-20 rounded-full bg-navy/10 border-2 border-navy/20 flex items-center justify-center text-3xl font-bold text-navy-light shadow-lg shadow-navy/5">
                  {user?.firstName?.[0] || "U"}
                </div>
              )}
              <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                >
                  <div className={`shrink-0 ${stat.color} bg-white rounded-lg p-2`}>
                    {stat.icon}
                  </div>
                  <div>
        ...
        <section className="max-w-4xl mx-auto px-6 py-10 space-y-8">
          <h2 className="text-lg font-semibold text-navy flex items-center gap-2">
            <BookOpen className="h-5 w-5 text-navy-light" />
            {t("myExams")}
          </h2>
          ...
          {purchases.length === 0 ? (
            <div className="bg-surface border border-border rounded-2xl p-14 text-center">
              ...
              <p className="text-navy font-semibold mb-1">{t("noExamsPurchased")}</p>
              <p className="text-text-muted text-sm mb-6">Your purchased exams will appear here.</p>
              <Link
                href="/exam"
                className="inline-flex items-center gap-2 text-sm bg-navy/5 hover:bg-navy/10 text-navy-light border border-navy/20 px-4 py-2 rounded-xl transition-colors"
              >
                {t("browseMockExams")}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-4 p-6 pb-5">
                    ...
                    {/* Best score display */}
                    {examBestScore > 0 && (
                      <div className="flex flex-col items-center justify-center shrink-0 bg-surface border border-border rounded-xl px-5 py-3 gap-2">
                  ...
                  <div className="flex flex-wrap items-center gap-3 px-6 pb-5">
                    ...
                    {latestCompleted && (
                      <Link
                        ...
                        className="inline-flex items-center gap-2 border border-border hover:border-navy/30 text-text-secondary hover:text-navy px-5 py-2.5 rounded-xl text-sm font-medium transition-colors"
                      >
                  ...
                  {completed.length > 0 && (
                    <div className="border-t border-border">
                      <div className="flex items-center justify-between px-6 pt-4 pb-3">
                        ...
                        {/* Column headers */}
                        <div className="hidden sm:flex items-center gap-8 pr-10 text-xs text-text-muted font-medium">
                        ...
                        {completed.map((attempt, i) => {
                          ...
                          return (
                            <Link
                              ...
                              className="flex items-center gap-4 px-6 py-3.5 hover:bg-surface transition-colors group"
                            >
                              {/* Attempt number */}
                              <span className="text-xs font-mono text-text-muted w-5 flex-shrink-0">
                                {completed.length - i}
                              </span>
                              ...
                              <div className="flex-1 flex items-center gap-2 min-w-0">
                                ...
                                {isBest && (
                                  <span className="text-xs text-yellow-600 bg-yellow-50 border border-yellow-200 px-1.5 py-0.5 rounded-full">
                                    Best
                                  </span>
                                )}
                              </div>
                              ...
                              <div className="hidden sm:flex items-center gap-8 flex-shrink-0">
                                <span className="w-10 text-center text-sm text-text-secondary tabular-nums">
                                  {attempt.scores?.rw ?? "—"}
                                </span>
                                <span className="w-10 text-center text-sm text-text-secondary tabular-nums">
                                  {attempt.scores?.math ?? "—"}
                                </span>
                                <span className={`w-14 text-center text-sm font-bold tabular-nums ${isBest ? "text-navy-light" : "text-navy"}`}>
                                  {attempt.scores?.total ?? "—"}
                                </span>
                              </div>

                              {/* Mobile score */}
                              <span className={`sm:hidden text-sm font-bold tabular-nums flex-shrink-0 ${isBest ? "text-navy-light" : "text-navy"}`}>
                                {attempt.scores?.total ?? "—"}
                              </span>

                              {/* Arrow */}
                              <ChevronRight className="h-4 w-4 text-text-muted group-hover:text-navy-light transition-colors flex-shrink-0" />
                            </Link>
                          );
                        })}
                      </div>
